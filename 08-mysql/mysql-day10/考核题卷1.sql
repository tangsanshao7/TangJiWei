...
-- # 10.请给创建好的表users添加一个字段，email，数据类型为varchar
--   (20)
...
--   ALTER TABLE users ADD datatime DATE;
...
--   UPDATE users SET datatime = '1999-06-15' WHERE NAME='赵六';
...
--   # 28.查询员工名
--   (first_name)中包含字符a的员工信息
--   SELECT *
--   FROM employees
--   WHERE first_name LIKE '%a%';

--   # 29.查询员工名
--   (first_name)中第3个字符为n，第五个字符为l的员工名和工资
...
--   IF (salary>10000,
--   IF (salary>15000,
--   IF (salary>20000,'A','B'),'C'),'D') AS 工资级别 FROM employees;

-- # 38.查询有奖金的每个领导手下员工的最高工资
--   SELECT MAX(salary) , manager_id
--   FROM employees
--   GROUP BY manager_id
--   HAVING manager_id IN (SELECT employee_id
--   FROM employees
...
--   HAVING manager_id IN ( SELECT DISTINCT manager_id
--     FROM employees
--     WHERE manager_id >102) AND MIN(salary)> 5000
...
--   FROM locations AS l JOIN departments AS d ON l.location_id= d.location_id;
...
--   # 45.查询员工名、部门名、工种名
--   (job_title)，并按部门名降序（三表连接）
--   SELECT first_name, department_name, job_title
--   FROM employees AS e JOIN departments AS d JOIN jobs AS j ON e.department_id= d.department_id AND e.job_id=j.job_id 
--   ORDER BY department_name DESC;


--   # 46.查询工资的个数>20的工资级别，并且按工资级别降序
--   SELECT *
--   FROM employees AS e JOIN job_grades AS g ON e.salary BETWEEN g.lowest_sal AND highest_sal
